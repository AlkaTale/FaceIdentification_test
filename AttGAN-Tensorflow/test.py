...
try:
    for idx, batch in enumerate(te_data):
        ...
        for i, b_sample_ipt in enumerate(b_sample_ipt_list):
            ...
            temp = sess.run(x_sample, feed_dict={xa_sample: xa_sample_ipt, _b_sample: _b_sample_ipt})
            sample = np.concatenate(temp, 2)
            save_dir = './output/%s/sample_testing/%d' % (experiment_name,i)
            im.imwrite(sample, '%s/%d.png' % (save_dir, idx))
